...
export const FriendsSideBar = () => {
    ...
    const fetchFriends = async () => {
      try {
        const result = await axios.get(
          `https://message-app-backend-production.up.railway.app/fetchFriends/${userId}`,
        );
  ...
  const openChat = (friendId: number, friendUsername: string) => {
        ...
        {friends.map((friend) => (
          <div key={friend.id} className="flex items-center gap-2  mb-2">
            <li
              ...
              className="cursor-pointer rounded p-2 flex-1 hover:bg-slate-800"
            >
              {friend.username}
            </li>
            <button
              onClick={() => removeFriend(friend.id)}
              className="hover:bg-red-700 hover:text-black px-2 border rounded"
            >
              X
            </button>
          </div>
